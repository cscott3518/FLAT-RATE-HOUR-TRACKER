...
function toCSV(rows) {
  ...
  const lines = [header.join(',')].concat(
    rows.map(r => [r.ro, r.date, r.description, r.hours].map(esc).join(','))
  );
  return lines.join('\n');
}
...
export default function App() {
  ...
  const addEntry = () => {
    ...
    setEntries(prev =>
      [row, ...prev].sort((a,b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 0))
    );
  ...
  const saveEdit = (patch) => {
    setEntries(prev =>
      prev.map(e => e.id === editing.id ? { ...e, ...patch, hours: parseFloat(patch.hours) } : e)
    );
    setEditing(null);
  };
  ...
  const importJSON = (file) => {
    ...
    reader.onload = () => {
      try {
        ...
        setEntries(merged);
      } catch { alert('Import failed. Provide a JSON export from this app.'); }
    };
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan="5" style={{textAlign:'center', padding:'18px 0', color:'#94a3b8'}}>
                      No entries yet. Add your first job above.
                    </td>
                  </tr>
                )}
                {filtered.map(r => (
